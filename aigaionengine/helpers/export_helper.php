...
require_once(APPPATH."include/utf8/trim.php");
	...
	// JAFMA 05/02/15
	if ( (!isset($fields['firstpage'])) && 
	     (!isset($fields['lastpage'])) &&
	     (strlen($publication->pages)>0) )
	{ // there are pages that are not being translated into the bibtex output
		$jafma_pps=explode('-',trim($publication->pages));
		if (count($jafma_pps)>=2)
		{
			$jafma_st=0;
			foreach ($jafma_pps as $jafma_item)
			{
				$jafma_v=intval($jafma_item);
				switch ($jafma_st)
				{
					case 0:{
						if ($jafma_v>0)
			...
			if (($jafma_st==2)&&($jafma_lp>=$jafma_fp))
				$fields['pages']=strval($jafma_fp).'--'.strval($jafma_lp);
		}
	}
